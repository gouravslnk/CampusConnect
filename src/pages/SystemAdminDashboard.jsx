...
export default function SystemAdminDashboard({ user }) {
  ...
  const stats = useMemo(() => {
    ...
    return [
      ...
      { label: 'Admins', value: users.filter((item) => item.role === 'admin' || item.role === 'club_admin').length, icon: Shield, tone: 'bg-slate-100 text-slate-700 dark:text-slate-200' },
    ];
  }, [clubs, events, users]);
  ...
  const renderClubs = () => (
    <div className="divide-y divide-slate-100">
      {filteredClubs.length === 0 ? (
        <div className="p-8 text-center text-sm text-slate-500 dark:text-slate-400">No hub requests match your search.</div>
          ...
          <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            <div>
              <div className="flex flex-wrap items-center gap-2">
                <button
                  ...
                  className="font-bold text-slate-900 dark:text-white text-left transition hover:text-cyan-700 disabled:cursor-default disabled:hover:text-slate-900 dark:text-white"
            ...
            <div className="flex gap-2">
              ...
              <button
                ...
                className="inline-flex items-center gap-2 rounded-lg border border-red-200 bg-white dark:bg-slate-900 px-3 py-2 text-sm font-semibold text-red-600 transition hover:bg-red-50 disabled:cursor-not-allowed disabled:opacity-50"
              >
                <X size={15} /> Reject
              </button>
              <button
              ...
              <td className="px-5 py-4">
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className="rounded-lg p-2 text-slate-500 dark:text-slate-400 transition hover:bg-slate-100 hover:text-slate-900 dark:text-white disabled:opacity-50"
                  ...
                  <button
                    ...
                    disabled={actionLoading === `event-${event.id}`}
                    className="rounded-lg p-2 text-slate-500 dark:text-slate-400 transition hover:bg-red-50 hover:text-red-600 disabled:opacity-50"
                    title="Remove event"
                  >
                    <Trash2 size={16} />
                  </button>
      ...
      <table className="w-full text-sm">
        ...
        <tbody className="divide-y divide-slate-100">
          {filteredUsers.length === 0 ? (
            <tr><td colSpan="6" className="px-5 py-8 text-center text-slate-500 dark:text-slate-400">No users match your search.</td></tr>
            ...
            <tr key={profile.id} className="hover:bg-slate-50/70">
              <td className="px-5 py-4">
                ...
                >
                  ...
                  <div>
                    <p className="font-semibold text-slate-900 dark:text-white">{profile.name || 'Unnamed user'}</p>
              ...
              <td className="px-5 py-4 text-slate-600 dark:text-slate-300">{formatDate(profile.created_at)}</td>
              <td className="px-5 py-4">
                <div className="flex items-center gap-2">
                  <button
                    onClick={() => setManageUser(profile)}
                    className="inline-flex items-center gap-1 rounded-lg border border-slate-200 bg-white dark:bg-slate-900 px-2.5 py-1.5 text-xs font-semibold text-slate-600 dark:text-slate-300 transition hover:bg-slate-50"
                  >
                  ...
                  <button
                    ...
                    className="inline-flex items-center gap-1 rounded-lg border border-red-200 bg-white dark:bg-slate-900 px-2.5 py-1.5 text-xs font-semibold text-red-600 transition hover:bg-red-50 disabled:cursor-not-allowed disabled:opacity-50"
                  >
    ...
    <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
      <div className="mb-8 flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">Approve hubs, remove events, and manage CampusConnect users.</p>
        </div>
        <div className="rounded-2xl border border-slate-200 bg-white dark:bg-slate-900 px-4 py-3">
          <p className="text-xs font-semibold uppercase tracking-wide text-slate-500 dark:text-slate-400">Signed in as</p>
          <p className="font-bold text-slate-900 dark:text-white">{user?.name || 'System Admin'}</p>
        </div>
      </div>
                ...
                return (
                  <button
                    ...
                    className={`inline-flex items-center gap-2 rounded-xl px-3 py-2 text-sm font-semibold transition ${
                      activeTab === tab.id ? 'bg-slate-900 text-white' : 'bg-slate-50 text-slate-600 dark:text-slate-300 hover:bg-slate-100'
                    }`}
                  >
      ...
      {manageUser && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 p-4 backdrop-blur-sm">
          <div className="w-full max-w-lg rounded-2xl bg-white dark:bg-slate-900 shadow-2xl overflow-hidden flex flex-col max-h-[80vh]">
            <div className="flex items-center justify-between border-b border-gray-100 dark:border-slate-800 px-6 py-4">
              <div>
                <h3 className="text-lg font-bold text-gray-900 dark:text-white">Manage Hub Admin</h3>
                <p className="text-sm text-gray-500 dark:text-gray-400">{manageUser.name || manageUser.email}</p>
              </div>
              <button
            ...
            <div className="p-6 overflow-y-auto">
              <h4 className="font-semibold text-slate-900 dark:text-white mb-3 text-sm uppercase tracking-wide">Approved Hubs</h4>
