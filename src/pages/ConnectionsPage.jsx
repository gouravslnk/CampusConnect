...
export default function ConnectionsPage() {
  ...
  const { accepted, incomingPending, outgoingPending, loading, hasFetched } = useSelector((state) => state.connections);
    ...
    return (
      <div className="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/50 p-4 shadow-sm hover:shadow-md transition-shadow">
        <div className="flex items-start gap-3">
          {profile.avatar ? (
            <img src={profile.avatar} alt={profile.name} loading="lazy" className="h-12 w-12 rounded-full object-cover ring-2 ring-slate-100" />
          ) : (
  ...
  if (loading) {
    return (
      <div className="mx-auto max-w-6xl px-4 py-10 sm:px-6 lg:px-8 animate-pulse">
        <div className="mb-8 rounded-3xl h-32 bg-slate-200 dark:bg-slate-800"></div>
        <div className="mb-3 h-6 w-40 bg-slate-200 dark:bg-slate-800 rounded"></div>
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2 mb-8">
          {[1, 2].map((i) => (
            <div key={i} className="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/50 p-4">
              <div className="flex items-start gap-3">
